...
<header class="container-fluid">
  ...
  include ('conexionDB.php');
    ...
    if (isset($res2['priv']) && $res2['priv'] == "adm" || $res2['priv'] == "editor") {
      echo "
...
<div class="col-sm-12"><a href="buscador.php"><input type="button" class="volver" value="Volver al buscador" ></a>
  <center>
  <form action="perfil2.php" method="get">
    <input type="text" name="Cedulaa" hidden value="<?=$_GET['Cedula']?>">

    <div class="col-sm-12"><img class="fotocarnet" src="../img/715.jpg" alt=""></div>
    <?php
    $x=1;
 include ('conexionDB.php');
      $result = mysql_query("SELECT * FROM alumnos WHERE Cedula='".$_GET['Cedula']."';");
      ...
      If($result)
          ...
          for($i=0; $i< $fcount; $i++)
               ...
               echo utf8_encode("<div class='col-sm-6 info'>
                ...
                mysql_query("ALTER TABLE alumnos CHANGE COLUMN ".$nombre." ".$_GET[$nombre]." VARCHAR(255);");
                echo "<script>";
              echo "window.location.href='perfil2.php?Cedula=".$_GET['Cedula1']."';";
              echo "</script>";
             ...
             if (isset($_GET[$nombre.$x])) {
               ...
               $que=mysql_query("UPDATE alumnos SET $nombre='".$_GET[$nombre.$x]."' WHERE Cedula='".$_GET['Cedulaa']."';");
               echo "<script>";
              echo "window.location.href='perfil2.php?Cedula=".$_GET['Cedula1']."';";
              echo "</script>";
              ...
              if (isset($_GET['campnew'])) {
                mysql_query("ALTER TABLE alumnos ADD ".$_GET['campnew']." varchar(255);");
              echo "<script>";
              echo "window.location.href='perfil2.php?Cedula=".$_GET['Cedula1']."';";
              echo "</script>";
              }
              ...
              if (isset($_GET['check'])) {
                mysql_query("ALTER TABLE alumnos DROP COLUMN ".$_GET['check'].";");
              echo "<script>";
              echo "window.location.href='perfil2.php?Cedula=".$_GET['Cedula1']."';";
              echo "</script>";
              }
...
<div class="col-sm-12 guard" hidden><input type="submit" hidden class="save" value="Guardar" hidden>
<input type="button" hidden class="saves" value="Cancelar" hidden></div>
</form>
</div>
